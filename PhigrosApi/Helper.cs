...
public static class Helper
{
	...
	public static async Task<(IActionResult?, T?)> ReadBodyAs<T, TController>(this TController controller, ILogger<TController> logger) where TController : Controller
	{
		...
		if (body == null)
		{
			logger.LogDebug("{ip} sent empty request.", controller.HttpContext.GetIP());
	...
	{
		logger.LogDebug("{ip} went error: {message}; {error}; {code}", controller.HttpContext.GetIP(), message, error, code);
